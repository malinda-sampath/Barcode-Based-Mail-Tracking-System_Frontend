...
export default function AdminManagement() {
  ...
  const handleDeleteMail = (mail: Mail) => {
    setMailData((prevMailData) =>
      prevMailData.filter((m) => m.mailId !== mail.mailId)
    );
  };
  ...
  return (
    <div className="m-12">
      ...
      <p className="text-xs sm:text-sm text-gray-500 text-[#611010] mb-4 ml-12">{currentDate}</p>
      ...
      {mailData.length > 0 && (
        ...
        <Table
          ...
          searchableKeys={["mailId", "sender", "recipient"]} // Add searchable keys
        />
        </div>
      )}
