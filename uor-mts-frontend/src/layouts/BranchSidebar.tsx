...
import { 
  BRANCH_DASHBOARD_SIDEBAR_LINKS, 
  BRANCH_DASHBOARD_SIDEBAR_BOTTOM_LINKS
...
const BranchSidebar: React.FC<SidebarProps> = ({ setIsSidebarOpen }) => {
  ...
  return (
    ...
    >
      ...
      <div className="flex-1 py-8 flex flex-col gap-0.5 overflow-y-auto">
        {BRANCH_DASHBOARD_SIDEBAR_LINKS.map((item) => (
      ...
      <div className="flex flex-col gap-0.5 pt-2 border-t border-[#F99C30]">
        {BRANCH_DASHBOARD_SIDEBAR_BOTTOM_LINKS.map((item) => (
          <SidebarLink key={item.key} item={item} isOpen={isOpen} />
        ))}
