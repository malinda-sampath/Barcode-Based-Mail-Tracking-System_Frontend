...
interface EmailEnterFormProps {
  ...
  onOTPSend: () => void; // Function prop to handle back navigation
}
...
const EmailEnterForm: React.FC<EmailEnterFormProps> = ({ onBack,setOTP,onOTPSend }) => {
    ...
    try {
      ...
      const response = await fetch(`${process.env.REACT_APP_API_URL}/password/get-otp/${email}`, {
        method: "POST",
